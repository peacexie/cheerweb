...
$_cbase['part']['def'] = 'cn'; // 默认:语言
$_cbase['part']['mods'] = array('about','news','cargo','gbook'); 
$_cbase['part']['psyn'] = array('about','cargo'); // 资料同步模型
// 请在相关模型主表的`xno`字段后,手动添加`part`字段
// 设置了psyn的模型，再添加psyn字段，用于同步
// mlang-start
if(isset($_cbase['tpl']['bdpart'])){
    $_bdpart = $_cbase['tpl']['bdpart'];
    if(isset($_cbase['part']['tab'][$_bdpart])){
        include DIR_ROOT."/extra/kvphp/lang-$_bdpart.php";
    }
}
// mlang-end
